...
This template makes a number of assumptions:

- You deploy to Kubernetes using Helm
- You use GitLab CI
- You publish Docker images to GCR
- You publish NuGet packages to Artifactory
- You run an Identity Server based on [Contrib.IdentityServer4.KubernetesStore](https://github.com/AXOOM/Contrib.IdentityServer4.KubernetesStore) in the same Kubernetes cluster

If any of these do not hold true, you'll need to make some modifications to projects generated by this template.

You will probably also need to search and replace the following strings:

- `eu.gcr.io/axoom-image`
- `https://axoom.jfrog.io/axoom/`
- `axoom-platform-*`
